...
FU_ARR *fearr_creat_arr(size_t s) {
// FU_ARR* fearr_creat_arr(size_t s, size_t type_size);
...
void *fearr_get(FU_ARR *arr, ArrInt index) {
...
int fearr_put(FU_ARR *arr, void* v) {
...
int fearr_set(FU_ARR *arr, ArrInt index, void* v) {
    if(arr==NULL) return -1;
...
void *fearr_del(FU_ARR *arr, ArrInt index) {
    if(arr==NULL) return NULL;
    void *temp;
    arr->used--;
    if(index==(arr->used + 1)) {
...
ArrInt fearr_len(FU_ARR *arr) {
    if(arr==NULL) return -1;
    return arr->used;
}

ArrInt fearr_size(FU_ARR *arr) {
    if(arr==NULL) return -1;
    return arr->size;
}

void fearr_free_container(FU_ARR *arr) {
    if(arr==NULL) return;
...
void fearr_free_all(FU_ARR *arr) {
    if(arr==NULL) return;
    if(arr->value==NULL) { free(arr); return; }
